...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Are you sure you want to leave?"):
        # This closes app
        self.master.destroy()
        os._exit(0)
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            cur.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))#,(var_fullname))
            count = cur.fetchone()[0]
            chkName = count
